...
\\
\subsection{k-$\boldsymbol{\epsilon}$ model}
The standard k-$\epsilon$ model equations is found in a lot of litterature such as \cite{CFD} and \cite{UNIK4900} and is specified as follows:\\
\\
...
\subsection{k-$\boldsymbol{\omega}$ SST}
K-$\omega$ model is made by Menter \cite{SST} and is found in littarture and such as \cite{CFD} and \cite{Wilcox}. Other turbulence resources such as \cite{NASA_SST} gives thurough desrcription of the model equations.\\
\\
...
\label{eqn:kOmegaTurbulentViscosity}
\end{equation}
Turbulent kinetic energy equation:
\begin{equation}
\frac{\partial \rho k}{\partial t} + U_j \frac{\partial \rho k}{\partial x_j} = \frac{\partial }{\partial x_j}\Big[\big(\mu + \sigma_k\mu_t\big)\frac{\partial k}{\partial x_j} \Big] + \rho\tau_{ij}\frac{\partial U_i}{\partial x_j}
\end{equation}
...
\\
For omega and epsilon, the wall functions \textit{omegaWallFunction} and \textit{epsilonLowReWallFunction} has been used. Both wall functions are applicable for a wide range og $y^+$ values.
