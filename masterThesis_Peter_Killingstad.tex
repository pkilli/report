...
\\
For velocity U, a typical dirichlet boundary conditions is a applid  at the inlet and on the barge. At the inlet , a \textit{fixedValue} is set, while a no-slip condition is set at the barge. The no-slip condition is a of a special kind, namely \textit{movingWallVelocity}, which is applied for "moving" walls when it is a moving reference frame.
\begin{landscape}
	\begin{table}[H]
		\centering
